...
func TestCreateManyPerfilMensalRepository(t *testing.T) {
	...
	mtestDb.Run("when_sending_a_valid_domain_returns_success", func(mt *mtest.T) {
		...
		perfilMensalEntity := []domain.PerfilMensalDomain{}
		err := faker.FakeData(&perfilMensalEntity)
		assert.Nil(t, err)

		err = repo.CreateManyPerfilMensalRepository(perfilMensalEntity)
		assert.Nil(t, err)
