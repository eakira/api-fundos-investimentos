...
type CdaBlc4Request struct {
	...
	DataCompetencia          string    `json:"DT_COMPTC,omitempty"`          // Data de competência do documento (AAAA-MM-DD)
	PrazoConfidencialidade   time.Time `json:"DT_CONFID_APLIC,omitempty"`    // Prazo de confidencialidade da aplicação (AAAA-MM-DD)
	DataFimVigencia          time.Time `json:"DT_FIM_VIGENCIA,omitempty"`    // Data fim da vigência (AAAA-MM-DD)
	DataInicioVigencia       time.Time `json:"DT_INI_VIGENCIA,omitempty"`    // Data início da vigência (AAAA-MM-DD)
	EmissorLigado            string    `json:"EMISSOR_LIGADO,omitempty"`     // Indica se o emissor da aplicação é ligado ao gestor ou administrador do fundo de investimento (S/N)
	QuantidadeAquisicoes     float64   `json:"QT_AQUIS_NEGOC,omitempty"`     // Quantidade de aquisições dos negócios realizados no mês
	QuantidadePosicaoFinal   float64   `json:"QT_POS_FINAL,omitempty"`       // Quantidade da posição final
	QuantidadeVendas         float64   `json:"QT_VENDA_NEGOC,omitempty"`     // Quantidade de vendas dos negócios realizados no mês
...
type CdaBlc5Request struct {
	...
	DataCompetencia               string    `json:"DT_COMPTC,omitempty"`          // Data de competência do documento (AAAA-MM-DD)
	PrazoConfidencialidade        time.Time `json:"DT_CONFID_APLIC,omitempty"`    // Prazo de confidencialidade da aplicação (AAAA-MM-DD)
	DataClassificacaoRisco        time.Time `json:"DT_RISCO,omitempty"`           // Data da classificação de risco (AAAA-MM-DD)
	DataVencimento                time.Time `json:"DT_VENC,omitempty"`            // Data de vencimento (AAAA-MM-DD)
	...
	PercentualCupomPosFixados     float64   `json:"PR_CUPOM_POSFX,omitempty"`     // Percentual do cupom (somente pós-fixados)
	PercentualIndexadorPosFixados float64   `json:"PR_INDEXADOR_POSFX,omitempty"` // Percentual do indexador (somente pós-fixados)
	PercentualTaxaPrefixada       float64   `json:"PR_TAXA_PREFX,omitempty"`      // Percentual da taxa contratada (somente pré-fixados)
	QuantidadeAquisicoes          float64   `json:"QT_AQUIS_NEGOC,omitempty"`     // Quantidade de aquisições dos negócios realizados no mês
	QuantidadePosicaoFinal        float64   `json:"QT_POS_FINAL,omitempty"`       // Quantidade da posição final
	QuantidadeVendas              float64   `json:"QT_VENDA_NEGOC,omitempty"`     // Quantidade de vendas dos negócios realizados no mês
...
type CdaBlc7Request struct {
	...
	DataCompetencia                   time.Time `json:"DT_COMPTC,omitempty"`              // Data de competência do documento (AAAA-MM-DD)
	PrazoConfidencialidade            time.Time `json:"DT_CONFID_APLIC,omitempty"`        // Prazo de confidencialidade da aplicação (AAAA-MM-DD)
	DataClassificacaoRisco            time.Time `json:"DT_RISCO,omitempty"`               // Data da classificação de risco (AAAA-MM-DD)
	DataVencimento                    time.Time `json:"DT_VENC,omitempty"`                // Data de vencimento (AAAA-MM-DD)
	NomeEmissor                       string    `json:"EMISSOR,omitempty"`                // Nome do Emissor (200 caracteres)
	...
	QuantidadeAquisicoes              float64   `json:"QT_AQUIS_NEGOC,omitempty"`         // Quantidade de aquisições dos negócios realizados no mês
	QuantidadeAtivosExterior          int       `json:"QT_ATIVO_EXTERIOR,omitempty"`      // Quantidade de ativos no exterior
	QuantidadePosicaoFinal            float64   `json:"QT_POS_FINAL,omitempty"`           // Quantidade da posição final
	QuantidadeVendas                  float64   `json:"QT_VENDA_NEGOC,omitempty"`         // Quantidade de vendas dos negócios realizados no mês
	RiscoEmissor                      string    `json:"RISCO_EMISSOR,omitempty"`          // Indica se o emissor do título possui classificação de risco (S/N)
	TipoAplicacao                     string    `json:"TP_APLIC,omitempty"`               // Tipo de aplicação (150 caracteres)
	...
	TipoNegociacao                    string    `json:"TP_NEGOC,omitempty"`               // Tipo de negociação (Para negociação/Mantido até o vencimento) (24 caracteres)
	ValorAquisicoes                   float64   `json:"VL_AQUIS_NEGOC,omitempty"`         // Valor das aquisições dos negócios realizados no mês
	ValorAtivoExterior                float64   `json:"VL_ATIVO_EXTERIOR,omitempty"`      // Valor do ativo no exterior
	ValorCustoPosicaoFinal            float64   `json:"VL_CUSTO_POS_FINAL,omitempty"`     // Valor de custo da posição final
	ValorMercadoPosicaoFinal          float64   `json:"VL_MERC_POS_FINAL,omitempty"`      // Valor de mercado da posição final
	ValorVendas                       float64   `json:"VL_VENDA_NEGOC,omitempty"`         // Valor das vendas dos negócios realizados no mês
}
...
type CdaConfidencial struct {
	...
	DenominacaoSocial   string    `json:"DENOM_SOCIAL,omitempty"`       // Denominação Social (100 caracteres)
	DataCompetencia     time.Time `json:"DT_COMPTC,omitempty"`          // Data de competência do documento (AAAA-MM-DD)
	PrazoConfidencial   time.Time `json:"DT_CONFID_APLIC,omitempty"`    // Prazo de confidencialidade da aplicação (AAAA-MM-DD)
	TipoAplicacao       string    `json:"TP_APLIC,omitempty"`           // Tipo de aplicação (150 caracteres)
	TipoFundo           string    `json:"TP_FUNDO,omitempty"`           // Tipo de fundo (15 caracteres)
	ValorAquisicoes     float64   `json:"VL_AQUIS_NEGOC,omitempty"`     // Valor das aquisições dos negócios realizados no mês
...
type CdaFiim struct {
	...
	CodigoPais               int       `json:"CD_PAIS,omitempty"`            // Código do país
	CodigoSELIC              string    `json:"CD_SELIC,omitempty"`           // Código SELIC (50 caracteres)
	CnpjFundo                string    `json:"CNPJ_FUNDO,omitempty"`         // CNPJ do fundo (20 caracteres)
	CpfCnpjEmissor           int       `json:"CPF_CNPJ_EMISSOR,omitempty"`   // Código de identificação do emissor, pessoa física ou jurídica
	...
	IDDocumento              int       `json:"ID_DOC,omitempty"`             // Identificador do documento
	...
	ValorCustoPosicaoFinal   float64   `json:"VL_CUSTO_POS_FINAL,omitempty"` // Valor de custo da posição final
	ValorMercadoPosicaoFinal float64   `json:"VL_MERC_POS_FINAL,omitempty"`  // Valor de mercado da posição final
	ValorPatrimonioLiquido   float64   `json:"VL_PATRIM_LIQ,omitempty"`      // Valor do patrimônio líquido
	ValorVendas              float64   `json:"VL_VENDA_NEGOC,omitempty"`     // Valor das vendas dos negócios realizados no mês
}
...
type CdaPatrimonioLiquido struct {
	...
	DataCompetencia   time.Time `json:"DT_COMPTC,omitempty"`     // Data de competência do documento (AAAA-MM-DD)
	TipoFundo         string    `json:"TP_FUNDO,omitempty"`      // Tipo de fundo (15 caracteres)
	ValorPatrimonio   float64   `json:"VL_PATRIM_LIQ,omitempty"` // Valor do patrimônio líquido (com precisão de 23 dígitos, 2 decimais)
}
